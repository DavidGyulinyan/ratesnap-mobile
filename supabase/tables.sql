...
CREATE TABLE IF NOT EXISTS public.multi_currency_converter_history (
    ...
    target_currencies JSONB NOT NULL, -- Array of {currency: string, amount: decimal, rate: decimal}
    conversion_results JSONB NOT NULL, -- Full conversion results
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.math_calculator_history (
    ...
    metadata JSONB, -- Additional data like memory, rounding, etc.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);

-- Create picked_rates table (for rates users have interacted with but not saved)
CREATE TABLE IF NOT EXISTS public.picked_rates (
    ...
    context JSONB, -- Additional context like amount converted, calculation result, etc.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);

-- Add RLS policies for multi_currency_converter_history
...
-- Add RLS policies for math_calculator_history
...
-- Add RLS policies for picked_rates
